...
- **Незамеченные (unmarked) строки должны совпадать с оригиналом дословно** — это и есть «контроль». Как и платформа, `-Check` не учитывает пробелы/табы и пустые строки (в т. ч. затёртые табы на пустых строках) — дрейфом считается только различие в коде или комментариях. Если оригинал в конфигурации-источнике изменится, unmarked-контекст разойдётся → метод потребует актуализации (см. ниже; проверить пачкой — `-Check`).
