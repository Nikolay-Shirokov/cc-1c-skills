...
### Заимствование общего модуля

Общий модуль заимствуют ради модуля расширения — другого содержимого у объекта нет. Создаётся:

1. **Метаданные** — `CommonModules/Имя.xml` с `ObjectBelonging=Adopted` и пометкой расширенного
   модуля: `<xr:PropertyState><xr:Property>Module</xr:Property><xr:State>Extended</xr:State>`
2. **Module.bsl** — пустой файл `CommonModules/Имя/Ext/Module.bsl`, куда пишутся перехватчики
   (`&Перед`, `&После`, `&Вместо`, `&ИзменениеИКонтроль`)
3. **Регистрация** — `<CommonModule>` в ChildObjects расширения

Повторное заимствование существующий `Module.bsl` не перезаписывает.
