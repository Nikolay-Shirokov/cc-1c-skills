...
| `check-typeset-coverage.mjs` | имена типов: всё, что печатает `meta-info`, принимает `meta-compile` (авторитет); карты не разошлись между портами |
...
`check-typeset-coverage.mjs` замыкает круг «прочитал вывод → подал на вход». `meta-info` —
читающий навык, и модель несёт его строки обратно в `meta-compile`; до гарда эта связь держалась на
честном слове и рвалась молча в обе стороны. `meta-info` печатал `ДокументОбъект.Заказ` в источниках
подписки, а компилятор отвечал «Неизвестный тип»: русских объектных имён в словаре не было — 33
метки из 41. И наоборот, в карте читалки не было `ConstantValueManager` и
`ChartOfCalculationTypesObject`, хотя компилятор их эмитит, — типы печатались по-английски посреди
русского вывода. Снэпшоты этого не видят: кейсы читают и компилируют по отдельности, круг не
замыкая.

Инвариант строгий в одну сторону: каждая метка, которую умеет печатать `meta-info`, обязана быть
ключом в `typeSynonyms` у `meta-compile` и вести ровно в тот же канон. Обратное неверно намеренно —
компилятор знает формы, которых читалка не печатает (английские, полные варианты рядом с
аббревиатурами). Заодно гард сверяет `objectTypeMap`/`refTypeMap` между портами навыка: значения
переводов — часть вывода, и расхождение портов дало бы разный текст на один и тот же файл.

Добавляешь вид в карту `meta-info` — заводи русскую метку в словарь `meta-compile` (и в копию
`meta-edit`, её держит `check-type-synonyms.mjs`).
...
| `{ "deletePath": "<путь>" }` | Удалить файл или каталог в `workDir` — состояние «файла нет», которое навыки сами не создают |
| `{ "git": ["<аргумент>", ...] }` | Выполнить `git` с аргументами в `workDir` — репозиторий для навыков, читающих изменения из git. Автор коммита задан, `core.autocrlf` выключен |
