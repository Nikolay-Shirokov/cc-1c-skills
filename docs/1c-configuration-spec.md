...
**Одна выгрузка — одна версия.** Штамп стоит в корне каждого XML выгрузки (`MetaDataObject`, `Form`, `Rights`, `CommandInterface`, `Help`, `Predefined` и др.), и у всех файлов он один — тот, что в `Configuration.xml`. Файл новее платформы она не читает («Неизвестная версия формата N загружаемого файла»); файл старее прочтёт, но выгрузка перестаёт быть однородной, а в PR такой файл выглядит как откат версии. Разъезжается обычно при слиянии веток, выгруженных разными платформами: новые файлы фичи приходят со штампом своей ветки без конфликта. Сверяют штамп с выгрузкой `form-validate` (проверка 14, `Ext/Form.xml`) и `meta-validate` (проверка 24, любой `MetaDataObject`, включая дескрипторы `Forms/*.xml` и `Templates/*.xml`); навыки-генераторы берут версию из выгрузки через `detect_format_version`.
