...
`<height>` — объявленная высота документа, и она **не обязана совпадать** с
фактическим числом `<rowsItem>`: в типовых макетах встречаются строки за
пределами объявленной высоты. При перегенерации макета объявленную высоту нужно
сохранять отдельно от подсчитанной, иначе геометрия меняется.
...
`<languageInfo>` повторяется по одному на язык. Многоязычный макет объявляет все
свои языки, и каждая локализованная строка (`<tl>`, `<format>`) несёт по
`<v8:item>` на язык:

```xml
<languageSettings>
    <currentLanguage>uk</currentLanguage>
...
Две ловушки при разборе макета:

- **Секции может не быть вовсе** — тогда язык определяется только по фактическому
  содержимому `<v8:lang>` в тексте ячеек.
- **`<currentLanguage>` не обязан совпадать с языком надписей.** Макет может
  объявлять один текущий язык, а хранить тексты под другим. Поэтому язык текстов
  надёжнее брать по факту — по тому, под каким `<v8:lang>` записано большинство
  надписей.

Язык, который встречается в тексте, но не объявлен в `languageSettings`,
платформа при загрузке разрешить не сможет и надписи на нём потеряет.
...
`<r>-1</r>` — объединение действует для всех строк, использующих данный набор колонок (аналог объединения колонок на уровне всего документа). Симметрично `<c>-1</c>` — для всех колонок.

Объединение с отрицательной координатой ни к какой строке не привязано и живёт в документе само по себе, поэтому при разборе макета его нельзя «повесить» на ячейку — оно требует отдельного представления.
...
| xsi:type                                | Значения                                     |
|-----------------------------------------|----------------------------------------------|
| `v8ui:SpreadsheetDocumentCellLineType`  | None, Solid, Dotted, Dashed, DashDotted, ... |
| `v8ui:SpreadsheetDocumentDrawingLineType` | Solid, None                                |

Атрибут `width` — толщина линии (1 = тонкая, 2 = толстая).

Элемент палитры — это пара «толщина + стиль», и обе части значимы: `width="1"
Solid` и `width="1" Dotted` — разные элементы с разными индексами. Поэтому у
ячейки, где стороны нарисованы по-разному (сплошная слева, пунктирная сверху),
односторонние ссылки формата смотрят в разные элементы палитры.
...
<!-- Системный шрифт: следует настройкам ОС, размера может не быть вовсе -->
<font ref="sys:DefaultGUIFont" faceName="" kind="WindowsFont"/>
```

| `kind`       | Смысл                                          |
|--------------|------------------------------------------------|
| `Absolute`   | Гарнитура и размер заданы явно                 |
| `StyleItem`  | Ссылка на элемент стиля конфигурации           |
| `WindowsFont`| Системный шрифт, размер берётся из настроек ОС |

Две особенности, на которых легко ошибиться:

- **`height` бывает дробным** (`8.3`, `6.8`) — округление до целого меняет вид
  макета. Записывается с точкой независимо от локали машины.
- **Пустой `faceName=""` — не то же самое, что отсутствующий**: он означает
  «наследовать гарнитуру по умолчанию», тогда как отсутствие атрибута оставляет
  выбор платформе.
...
<format>
    ...
    <borderColor>#000000</borderColor>      <!-- цвет рамки -->
    <width>24</width>                       <!-- ширина (для колонок) -->
    <height>84</height>                     <!-- высота (для строк) -->
    <horizontalAlignment>Center</horizontalAlignment>   <!-- Left | Center | Right | Justify | ... -->
    <verticalAlignment>Center</verticalAlignment>       <!-- Top | Center | Bottom -->
    <textPlacement>Wrap</textPlacement>     <!-- Wrap | Auto | Cut -->
    <textColor>#FF0000</textColor>          <!-- цвет текста -->
    <hidden>false</hidden>                  <!-- скрытая ячейка -->
    <indent>2</indent>                      <!-- отступ текста -->
...
Значения выравнивания и размещения текста платформа пишет своими токенами, и
список ими не исчерпывается (`Justify`, `Fill` и др.) — при разборе макета
разумнее пробрасывать токен как есть, а не сводить к замкнутому перечислению.

### Рамка по всем сторонам — `<border>`

Когда все четыре стороны нарисованы одной линией, платформа пишет **один**
`<border>` вместо четырёх одинаковых односторонних элементов:

```xml
<format>
    <font>0</font>
    <border>0</border>          <!-- индекс линии для всех четырёх сторон -->
</format>
```

Односторонние элементы появляются только там, где стороны различаются:

```xml
<format>
    <leftBorder>1</leftBorder>      <!-- Solid  width=1 -->
    <topBorder>0</topBorder>        <!-- Dotted width=1 -->
    <rightBorder>2</rightBorder>    <!-- Solid  width=2 -->
    <bottomBorder>3</bottomBorder>  <!-- Dotted width=2 -->
</format>
```

`<border>` ссылается на палитру линий так же, как односторонние элементы, —
проверка индексов должна учитывать и его.

### Ячейка-поле ввода

Ячейка, редактируемая пользователем при просмотре табличного документа (так
устроены формы регламентированных отчётов). Описывается тройкой свойств формата:
...
| Свойство         | Описание                                                    |
|------------------|-------------------------------------------------------------|
| `<containsValue>`| `true` — ячейка хранит значение                             |
| `<valueType>`    | Тип значения в нотации описания типов 1С                    |
| `<controlType>`  | GUID вида элемента управления                               |

GUID видов элементов управления:

| GUID                                   | Вид элемента |
|----------------------------------------|--------------|
| `381ed624-9217-4e63-85db-c4c3cb87daae` | Поле ввода   |
| `35af3d93-d7c7-4a2e-a8eb-bac87a1a3f26` | Флажок       |

Квалификаторы по типу значения:

| `v8:Type`     | Квалификаторы                                          |
|---------------|--------------------------------------------------------|
| `xs:decimal`  | `Digits`, `FractionDigits`, `AllowedSign`              |
| `xs:string`   | `Length` (0 = неограниченная), `AllowedLength`         |
| `xs:dateTime` | `DateFractions` (`Date`, `Time`, `DateTime`)           |
| `xs:boolean`  | —                                                      |

Поле ввода совместимо с содержимым: ячейка может одновременно нести `<tl>`
(показываемое значение) и быть редактируемой.
