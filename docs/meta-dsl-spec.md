...
- **Ключа нет** → блок не эмитится (обычный каталог: Наименование, Код, Владелец и т.д. остаются платформенными по умолчанию).
...
Тип кода в XML определяется свойством каталога `codeType`: `Number` → `<Code xsi:type="xs:decimal">`, `String` → `<Code>` (без типа); пустой код → `<Code/>`.
