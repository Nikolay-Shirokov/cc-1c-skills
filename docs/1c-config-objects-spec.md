...
Свойство «Режим сокращения типа», три значения:

| XML | В конфигураторе |
|-----|-----------------|
| `TransformValues` | Преобразовывать значения (по умолчанию) |
| `DeleteData` | Удалять данные |
| `Deny` | Запрещать |

У измерений его пишут только регистры сведений. `DeleteData` в выгрузках редок (7 измерений на всю УТ 11.5.27, ни одного в БП) и принимается платформами 8.3.25–8.3.27 — тот же порог, что у самого свойства.

Свойство именно новое: платформа 8.3.24 при загрузке молча его отбраковывает («Свойство не входит в состав объекта метаданных»), сообщая при этом об успехе.

**Изменения в версии 2.19 (платформа 8.3.26):** новых элементов нет; изменилась только выгрузка ролей — право, совпадающее с `setForNewObjects`, больше не пишется (см. [1c-role-spec.md](1c-role-spec.md#версии-формата)).
