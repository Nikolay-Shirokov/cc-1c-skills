...
Адреса привязки, вписанные в глобальный блок вручную (`Listen 127.0.0.1:8081` и `Listen [::1]:8081`), при повторной публикации сохраняются все — меняется только порт; URL в выводе `web-publish` и `web-info` строятся по первому из них. `0.0.0.0`, `*`, `[::]` считаются «все интерфейсы» → `localhost`.
