...
| `childObjectsOrder` | `"append"`/`"alphabetical"` | нет | `append` | Куда навыки вставляют новый объект в `<ChildObjects>` `Configuration.xml` (см. ниже) | Руками |
...
| `childObjectsOrder` | `"append"`/`"alphabetical"` | нет | Override порядка вставки в `<ChildObjects>` для этой базы (см. ниже) | Руками |
...
### Порядок вставки в `ChildObjects` и `childObjectsOrder`

Навыки, создающие объекты верхнего уровня (`meta-compile`, `role-compile`, `subsystem-compile`, `xdto-compile`), регистрируют их в `<ChildObjects>` файла `Configuration.xml` (подсистема — в `ChildObjects` родительской подсистемы). Куда ставится новая запись, задаёт `childObjectsOrder`:
- `append` (по умолчанию, в т.ч. когда поле не задано) — после последнего объекта того же типа, как дописывает сам Конфигуратор;
- `alphabetical` — по имени среди объектов того же типа, как требует стандарт разработки для объектов верхнего уровня (АПК:1108 «Нарушена сортировка объектов метаданных верхнего уровня по имени», #std467 п. 2.3). Типовые выгрузки (ЗУП, КА, ERP) держат этот порядок; нарушают его именно дописанные в конец объекты.

Порядок имён — как в дереве Конфигуратора: регистр не учитывается, подчёркивание раньше цифр, цифры раньше букв, латиница раньше кириллицы, `ё` на месте `е`. Сравнение реализовано ключом «ранг+символ», а не культурными таблицами ОС, поэтому PowerShell- и Python-порты дают одинаковый результат на любой платформе. Взаимное расположение групп типов навыки не меняют — новый тип встаёт в конец блока.

Оба порта правят `Configuration.xml` как текст (DOM — только на чтение, для поиска `ChildObjects` и отсечения дубликата): файл меняется ровно на одну вставленную строку, заголовок, самозакрывающиеся теги, BOM и переводы строк остаются как были — даже если выгрузка не в каноне Конфигуратора.

Раскладка та же, что у `editingAllowedCheck`: `databases[].childObjectsOrder` базы, чей `configSrc` (относительно каталога `.v8-project.json`) охватывает каталог редактируемого XML; иначе корневое; иначе `append`. Файл `.v8-project.json` ищется вверх от каталога конфигурации, затем от текущего каталога.

Порядок подсистем в дереве — только значение по умолчанию для панели разделов: явный порядок задаёт `<SubsystemsOrder>` в `Ext/CommandInterface.xml` конфигурации (для вложенных — родительской подсистемы). Алфавит в дереве его не трогает; не перечисленные там подсистемы платформа показывает в порядке дерева после перечисленных.

`cf-edit` и `cfe-borrow` (включение существующего объекта в состав конфигурации/расширения) всегда вставляют по алфавиту — настройка на них не влияет.
