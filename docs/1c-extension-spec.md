...
9. **Картинки элементов** (сверено с Конфигуратором 8.3.27):
   - у `PictureDecoration` блок `<Picture>` удаляется в обеих секциях при любом виде картинки (`xr:Abs`, `CommonPicture`, `StdPicture`). Сам элемент остаётся;
   - `StdPicture.*` у кнопок, подменю и страниц **сохраняется** в обеих секциях как есть;
   - блоки `<Picture>` с `<xr:Ref>CommonPicture.XXX</xr:Ref>` удаляются, только если `CommonPicture.XXX` не заимствуется (нет в конфигурации-источнике).

10. **Авто-заимствование CommonPictures** — при заимствовании формы автоматически заимствуются все CommonPictures, на которые ссылаются элементы формы (картинки декораций к этому моменту уже удалены и заимствования не вызывают).
...
13. **Файлы встроенных картинок** — ссылка `<xr:Abs>Файл</xr:Abs>` в свойстве элемента (`Picture`, `RowsPicture`, `HeaderPicture`, `ValuesPicture`) указывает на `Ext/Form/Items/<имя элемента>/<Файл>`. Файлы, на которые ссылается итоговая форма, копируются из источника байт в байт; без них платформа отвергает загрузку («Файл не найден»). Уже лежащий в расширении файл при повторном заимствовании не перезаписывается.
...
| `Extended` | Свойство расширено: у объекта появился модуль или изменена форма |

#### Extended — модули и формы заимствованных объектов

`Extended` стоит на самом заимствованном объекте (а не на реквизите) и означает, что расширение
принесло содержимое свойства. Имя свойства совпадает с **базовым именем файла модуля**:

| Файл в расширении | `xr:Property` |
|-------------------|---------------|
| `Ext/Module.bsl` (общий модуль, HTTP- и web-сервис) | `Module` |
| `Ext/ObjectModule.bsl` | `ObjectModule` |
| `Ext/ManagerModule.bsl` | `ManagerModule` |
| `Ext/RecordSetModule.bsl` | `RecordSetModule` |
| `Ext/ValueManagerModule.bsl` | `ValueManagerModule` |
| `Commands/<Имя>/Ext/CommandModule.bsl` | `CommandModule` — на вложенном `<Command>`, не у владельца |
| заимствованная форма | `Form` — в метаданных формы |

Порядок внутри `InternalInfo`: после всех `xr:GeneratedType`, сначала «объектный» модуль
(`ObjectModule` / `RecordSetModule` / `ValueManagerModule`), затем `ManagerModule`.

**Версионность.** Элемент появился в версии формата **2.19 (8.3.26)**. На 2.18 и ниже платформа
молча выбрасывает его при загрузке. Начиная с 2.19 Конфигуратор проставляет пометку сам при
выгрузке — триггером служит наличие файла модуля, даже пустого; заимствованный объект без модуля
её не получает. Расхождение (файл без пометки или пометка без файла) платформа принимает без
ошибок, но выгрузка Конфигуратора так не выглядит.
