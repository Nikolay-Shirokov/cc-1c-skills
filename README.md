...
| Утилиты | `/img-grid`, `/syntax-help` | Наложение сетки на изображение для определения пропорций колонок; поиск и чтение справки установленной платформы: синтакс-помощник, ключи пакетного режима, конфигуратор | — |

## Требования

- **Windows** с PowerShell 5.1+ (входит в Windows) — рантайм по умолчанию
- **1С:Предприятие 8.3** — для сборки/разборки EPF/ERF (навыки генерации XML работают без платформы); `/syntax-help` читает файлы справки установленной платформы, не запуская её
...
└── syntax-help/         # Справка платформы: поиск и чтение
scripts/
